...
class DeathFinder():
  ...
  def __init__(self, width, height, broadinfo, wait=5.0):

    assert type(width) is int
    assert type(height) is int
    assert len(broadinfo) is 3
    ...
    self.wait = wait
    self.broaddress = broadinfo[0]
    self.brosend = broadinfo[1]
    self.broreci = broadinfo[2]
    self.manager = BroadcastServer((self.broaddress, self.broreci), wait_time=wait)
  ...
  def BroadcastUpdate(self):
    ...
    ....@....!n
    """
    becon = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    becon.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
    becon_addr = (self.broaddress, self.brosend)
...
class BroadcastServer():
  ...
  def __init__(self, broadcast, wait_time=5.0, _epoch=0, log_buffer=sys.stderr, logging=True):

    assert len(broadcast) is 2
    ...
    self.ingress.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
    self.ingress.bind(broadcast)
    self.ingress.settimeout(0.5)

    print("Death Finder Server Listening %s:%s"%(broadcast[0], broadcast[1]), file=log_buffer)
  ...
  def listen(self):
    ...
    if True: #try:
      while time.time() < wait_time:
        try:
          clientdata, addr = self.ingress.recvfrom(30)
...
if __name__ == "__main__":
  ...
  recieving = 5003
  sending   = 5002
  Game = DeathFinder(40,200, (address, sending, recieving), wait=2)
